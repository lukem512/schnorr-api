...
void load_curve(Integer& q,ECP& ec,ECPPoint& G)
{
	// Load in curve secp256r1
	Integer p, a, b, Gx, Gy;
...
void KeyGen(Integer& sk, Integer& pk1, Integer& pk2, AutoSeededRandomPool& rng)
{
	...
	sk = Integer(rng, 256) % q;
	Q = ec.ScalarMultiply(G, sk);
	...
	const byte* message, int mlen)
{
	...
	int len = ec.EncodedPointSize();
	byte *buffer = new byte[len];
	ec.EncodePoint(buffer, R, false); // set this to true to compress
	sha.Update(buffer, len);
	delete[] buffer;
	      ...
	      const byte* message, int mlen, AutoSeededRandomPool& rng)
{
	...
	k = Integer(rng, 256) % q;
	R = ec.ScalarMultiply(G, k);
	sig1 = HashPointMessage(ec, R, message, mlen)%q;
	sig2 = (k - sk*sig1) % q;
}
	        ...
	        const byte* message,int mlen)
{
	...
	Q = ECPPoint(pk1,pk2); 
	R = ec.CascadeScalarMultiply(G, sig2, Q, sig1);

	Integer sig1d = HashPointMessage(ec, R, message, mlen) % q;
	return (sig1==sig1d);
}
